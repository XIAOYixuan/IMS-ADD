...
@BackendRegistry.register("aasist")
class AASIST(BaseBackendModel):
    ...
    def forward(self, audio_batch: AudioBatch, frontend_output: FrontendOutput) -> BackendOutput:
        last_hidden_states = frontend_output.foutput.last_hidden_state # type: ignore
        #print('shape', last_hidden_states.shape)
        #-------pre-trained Wav2vec model fine tunning ------------------------##
        #x_ssl_feat = self.ssl_model.extract_feat(x.squeeze(-1))
        x = self.LL(last_hidden_states) #(bs,frame_number,feat_out_dim)
...
if __name__ == '__main__':
    ...
    model.eval()
    with torch.no_grad():
        model_out: BackendOutput = model(audio_batch, frontend_out)
