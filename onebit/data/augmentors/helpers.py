...
def align_waveform(waveform: np.ndarray,
                   ...
                   start_index: Optional[int] = None) -> np.ndarray:
    """
...
def compute_amplitude(waveform: np.ndarray, 
    ...
    # adapted from speechbrain's impl
    ...
    if scale == "linear":
        ...
        return float(np.clip(20 * np.log10(out), -80, None))  # type: ignore
...
def select_audio_from_df(df: pd.DataFrame, 
                          sample_rate: int = 16000) -> np.ndarray:
    # adapted from speechbrain's impl
    path = df.sample(1)["path"].values[0]
    audio, sr = librosa.load(path, sr=sample_rate, mono=True)
...
def select_multiple_audio_from_df(df: pd.DataFrame,
                                   count: int, 
                                   sample_rate: int = 16000) -> List[np.ndarray]:
    # adapted from speechbrain's impl
    import logging
...
def load_csv_dataframe(csv_file: str) -> pd.DataFrame:
    return pd.read_csv(csv_file, header=None, names=['path'])
